...
# 파일에서 객체 로드
label_encoder = joblib.load('/app/work/label_encoder_classes.joblib')
...
# 모델 학습
model = load('/app/work/lgbm_model.pkl')
preds = model.predict(air_df)
#모델 저장 
joblib.dump(model, '/app/work/lgbm_model.pkl')
#예측데이터 프레임 생성
air_df['traffic']=preds
air_df['gu'] = label_encoder.inverse_transform(air_df['gu'])
gu_df = pd.read_csv('/app/work/spot_data_guz.csv')
cols ={ 'spot_num' : 'spot' }
...
row = l_df.iloc[0]  # 첫 번째 행만 사용
insert_query = f"INSERT INTO TABLE default.predict VALUES ('{row['gu']}', '{row['ymd']}', {row['hour']}, {row['traffic']})"
